...
EIDIR = `erl -noshell -eval 'io:format("~n~s~n", [code:lib_dir("erl_interface")]).' -s erlang halt | tail -n 1`.chomp

$C_FLAGS="-I#{EIDIR}/include -L#{EIDIR}/lib "

if RUBY_PLATFORM =~ /darwin/i
	$C_FLAGS += "-fno-common -Wall -Wl,-bundle,-undefined,dynamic_lookup -fPIC -I#{ROOTDIR}/usr/include -lei"
elsif RUBY_PLATFORM =~ /linux/i or RUBY_PLATFORM =~ /netbsd/i
	$C_FLAGS += "-shared -Wall -I#{ROOTDIR}/usr/include -fPIC -lei"
else
	$C_FLAGS += "-shared -Wall -I#{ROOTDIR}/usr/include -fPIC -lei"
end
...
rule ".so" => ["%{priv,c_src}X.c"] + C_HEADERS do |t|
	sh "gcc #{t.source} #{$C_FLAGS} -o #{t.to_s}"
end
...
task :compile => [:contrib, 'ebin', 'priv'] + HEADERS + OBJ + ['priv/syslog_drv.so'] + RELEASE do
...
task :install => [:compile] do
	sh "mkdir #{ROOTDIR}/lib/syslog-1.0" unless File.directory? "#{ROOTDIR}/lib/syslog-1.0"
	sh "cp -r src ebin c_src priv #{ROOTDIR}/lib/syslog-1.0"
end
...
namespace :test do
	...
	task :compile => [:contrib, 'debug_ebin'] + HEADERS + DEBUGOBJ + ['priv/syslog_drv.so']
